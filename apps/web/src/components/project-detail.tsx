    ...
    <section className="mt-4 space-y-5">
      ...
      <div className="mt-3">
        <div className="flex max-w-full flex-wrap items-center gap-2 text-xs">
          <MetaChip
            icon={<span className="text-muted-foreground">Code:</span>}
            value={project.code}
            withSeparator
          />
          <MetaChip
...
function getFileIcon(type: FileType) {
  switch (type) {
    ...
    default:
      return (
        <IconFile className="size-8 text-muted-foreground" aria-hidden="true" />
      )
  }
}
...
function MetaRow({
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex w-30 shrink-0 items-center gap-2 text-sm text-muted-foreground">
        {icon}
        <span>{label}</span>
      </div>
